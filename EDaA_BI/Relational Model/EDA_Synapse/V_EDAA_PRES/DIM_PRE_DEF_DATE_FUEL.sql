CREATE VIEW [V_EDAA_PRES].[DIM_PRE_DEF_DATE_FUEL]  AS
WITH variables
AS (
   SELECT
       -- Daily Element
...
AS (SELECT format(MIN(Cal_Dt), 'yyyyMMdd') AS FscYrBegin,
    ...
    GROUP BY Fsc_Yr
     ...
     ly_fsc_lookup
AS (SELECT Format(Lst_Yr_Fsc_Dt, 'yyyyMMdd') AS LastYrFscDate,
    Format(DATEADD(day, 1 - DATEPART(WEEKDAY, Lst_Yr_Fsc_Dt) % 7, Lst_Yr_Fsc_Dt), 'yyyyMMdd') AS LYWeekStart,
    Format(DATEADD(day, 7 - DATEPART(WEEKDAY, Lst_Yr_Fsc_Dt) % 7, Lst_Yr_Fsc_Dt), 'yyyyMMdd') AS LYWeekEnd
    FROM EDAA_DW.DIM_DLY_CAL WITH (NOLOCK)
    WHERE Dt_Sk = CONVERT(varchar(8), GETDATE(), 112)
   ),
     day_lookup
AS (
...
UNION ALL
SELECT 'Trailing 4 Weeks' AS PRE_DEF_NM,
        36 AS PRE_DEF_ID,
        'AGG' AS AGG_TYPE,
        format(DATEADD(week, -4, DATEADD(day, -1, CurrentDate)), 'yyyyMMdd') AS MIN_DT_SK,
        format(DATEADD(day, -1, CurrentDate), 'yyyyMMdd') AS MAX_DT_SK
FROM variables
UNION ALL
SELECT 'LY Trailing 4 Weeks' AS PRE_DEF_NM,
        37 AS PRE_DEF_ID,
        'AGG' AS AGG_TYPE,
        format(DATEADD(week, -4, DATEADD(day, -1, LastYrFscDate)), 'yyyyMMdd') AS MIN_DT_SK,
        format(DATEADD(day, -1, LastYrFscDate), 'yyyyMMdd') AS MAX_DT_SK
FROM ly_fsc_lookup
UNION ALL
SELECT 'LYWTD' AS PRE_DEF_NM,
        38 AS PRE_DEF_ID,
        'AGG' AS AGG_TYPE,
        LYWeekStart AS MIN_DT_SK,
        FORMAT(DATEADD(day, -1, LastYrFscDate), 'yyyyMMdd') AS MAX_DT_SK
FROM ly_fsc_lookup
UNION ALL
SELECT 'LY Last 7 days' AS PRE_DEF_NM,
       39 AS PRE_DEF_ID,
       'AGG' AS AGG_TYPE,
       Format(DATEADD(day, -7, LastYrFscDate), 'yyyyMMdd') AS MIN_DT_SK,
       format(DATEADD(day, -1, LastYrFscDate), 'yyyyMMdd') AS MAX_DT_SK
FROM ly_fsc_lookup
UNION ALL
SELECT 'LY Next 14 days' AS PRE_DEF_NM,
       40 AS PRE_DEF_ID,
       'AGG' AS AGG_TYPE,
       LastYrFscDate AS MIN_DT_SK,
       format(DATEADD(day, 14, LastYrFscDate), 'yyyyMMdd') AS MAX_DT_SK
FROM ly_fsc_lookup
UNION ALL
SELECT *
FROM day_lookup;
