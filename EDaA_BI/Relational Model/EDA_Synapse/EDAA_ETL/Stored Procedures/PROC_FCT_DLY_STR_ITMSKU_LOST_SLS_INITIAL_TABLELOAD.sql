CREATE PROC [EDAA_ETL].[PROC_FCT_DLY_STR_ITMSKU_LOST_SLS_INITIAL_TABLELOAD] AS
...
FROM  EDAA_STG.DLY_UT_P_UPC_HST_LOSTSALES
...
@SCHEMA = 'EDAA_DW',
@TABLENAME='FCT_DLY_STR_ITMSKU_LOST_SLS',
@PARTITIONVALUE = @PARTITIONDATESTARTKEY,
@PARTITIONNUMBER = @PNUMBER  OUTPUT ;
SELECT @PNUMBER


IF OBJECT_ID('EDAA_STG.FCT_DLY_STR_ITMSKU_LOST_SLS_OUT') IS NOT NULL
BEGIN
    DROP TABLE EDAA_STG.FCT_DLY_STR_ITMSKU_LOST_SLS_OUT
END


CREATE TABLE EDAA_STG.FCT_DLY_STR_ITMSKU_LOST_SLS_OUT
WITH
(
...
AS
SELECT *
FROM    EDAA_DW.FCT_DLY_STR_ITMSKU_LOST_SLS
WHERE 1=2
OPTION (LABEL = 'CTAS : PARTITION SWITCH IN : UPDATE')
;

IF OBJECT_ID('EDAA_STG.PK_FCT_DLY_STR_ITMSKU_LOST_SLS_OUT') IS NULL
BEGIN
DECLARE @SQL_ALTER1 NVARCHAR(4000) = '
ALTER TABLE EDAA_STG.FCT_DLY_STR_ITMSKU_LOST_SLS_OUT  ADD CONSTRAINT PK_FCT_DLY_STR_ITMSKU_LOST_SLS_OUT
...
IF OBJECT_ID('EDAA_STG.FCT_DLY_STR_ITMSKU_LOST_SLS_IN') IS NOT NULL
BEGIN
    DROP TABLE EDAA_STG.FCT_DLY_STR_ITMSKU_LOST_SLS_IN
END

CREATE TABLE EDAA_STG.FCT_DLY_STR_ITMSKU_LOST_SLS_IN
WITH
(
	DISTRIBUTION = HASH ( DT_SK ),
	CLUSTERED COLUMNSTORE INDEX,
...
IF OBJECT_ID('EDAA_STG.PK_FCT_DLY_STR_ITMSKU_LOST_SLS_IN') IS NULL
BEGIN
DECLARE @SQL_ALTER NVARCHAR(4000) = '
ALTER TABLE EDAA_STG.FCT_DLY_STR_ITMSKU_LOST_SLS_IN  ADD CONSTRAINT PK_FCT_DLY_STR_ITMSKU_LOST_SLS_IN
PRIMARY KEY NONCLUSTERED
(
		DT_SK ASC,
		GEO_HIST_SK ASC,
...
--select * from  EDAA_STG.FCT_DLY_STR_ITMSKU_LOST_SLS_OUT ;

PRINT('DATA LOAD IN _IN TEMP TABLE');

INSERT INTO EDAA_STG.FCT_DLY_STR_ITMSKU_LOST_SLS_IN
...
PRINT('DATA LOAD DONE IN _IN TEMP TABLE');

BEGIN

--SELECT @NBR_OF_RW_ISRT = COUNT(1)  FROM EDAA_STG.FCT_DLY_STR_ITMSKU_LOST_SLS_IN WHERE AUD_INS_SK = @NEW_AUD_SKY
--SELECT @NBR_OF_RW_UDT  = COUNT(1)  FROM EDAA_STG.FCT_DLY_STR_ITMSKU_LOST_SLS_IN WHERE AUD_UPD_SK = @NEW_AUD_SKY

SET @NBR_OF_RW_UDT_TOTAL = @NBR_OF_RW_UDT_TOTAL + 1
SET @NBR_OF_RW_ISRT_TOTAL = @NBR_OF_RW_ISRT_TOTAL + 1

PRINT('PARTITION SWITCH STARTED');

--SWITCH THE PARTITIONS OVER
DECLARE @SQL NVARCHAR(4000) = '
ALTER TABLE EDAA_DW.FCT_DLY_STR_ITMSKU_LOST_SLS SWITCH PARTITION '+CAST(@PNUMBER AS VARCHAR(20))    +' TO EDAA_STG.FCT_DLY_STR_ITMSKU_LOST_SLS_OUT PARTITION '    +CAST(@PNUMBER AS VARCHAR(20))+';
ALTER TABLE EDAA_STG.FCT_DLY_STR_ITMSKU_LOST_SLS_IN SWITCH PARTITION '+CAST(@PNUMBER AS VARCHAR(20))    +' TO EDAA_DW.FCT_DLY_STR_ITMSKU_LOST_SLS PARTITION '        +CAST(@PNUMBER AS VARCHAR(20))+';'
EXEC SP_EXECUTESQL @SQL

PRINT('PARTITION SWITCH COMPLETED');

--PERFORM THE CLEAN-UP
TRUNCATE TABLE EDAA_STG.FCT_DLY_STR_ITMSKU_LOST_SLS_OUT;
TRUNCATE TABLE EDAA_STG.FCT_DLY_STR_ITMSKU_LOST_SLS_IN;
