CREATE PROC [EDAA_ETL].[PROC_FCT_DLY_STR_ITMSKU_SLS_INITIAL_TABLELOAD] AS
...
EXEC EDAA_CNTL.SP_AUDIT_DATA_LOAD_START
 @EXEC_LYR  = 'EDAA_DW'
,@EXEC_JOB  = 'PROC_FCT_DLY_STR_ITMSKU_SLS_INITIAL_TABLELOAD'
,@SRC_ENTY  = 'UT_UPC_DLY_SL_HST_INITIAL'
...
FROM  EDAA_STG.UT_UPC_DLY_SL_HST_INITIAL
...
@SCHEMA = 'EDAA_DW',
@TABLENAME='FCT_DLY_STR_ITMSKU_SLS',
@PARTITIONVALUE = @PARTITIONDATESTARTKEY,
@PARTITIONNUMBER = @PNUMBER  OUTPUT ;
SELECT @PNUMBER


IF OBJECT_ID('EDAA_STG.FCT_DLY_STR_ITMSKU_SLS_OUT') IS NOT NULL
BEGIN
    DROP TABLE EDAA_STG.FCT_DLY_STR_ITMSKU_SLS_OUT
END


CREATE TABLE EDAA_STG.FCT_DLY_STR_ITMSKU_SLS_OUT
WITH
(
...
IF OBJECT_ID('EDAA_STG.FCT_DLY_STR_ITMSKU_SLS_IN') IS NOT NULL
BEGIN
    DROP TABLE EDAA_STG.FCT_DLY_STR_ITMSKU_SLS_IN
END

CREATE TABLE EDAA_STG.FCT_DLY_STR_ITMSKU_SLS_IN
WITH
(
	DISTRIBUTION = HASH ( DT_SK ),
	CLUSTERED COLUMNSTORE INDEX,
...
IF OBJECT_ID('EDAA_STG.PK_FCT_DLY_STR_ITMSKU_SLS_IN') IS NULL
BEGIN
DECLARE @SQL_ALTER NVARCHAR(4000) = '
ALTER TABLE EDAA_STG.FCT_DLY_STR_ITMSKU_SLS_IN  ADD CONSTRAINT PK_FCT_DLY_STR_ITMSKU_SLS_IN
...
IF OBJECT_ID('EDAA_STG.PK_FCT_DLY_STR_ITMSKU_SLS_OUT') IS NULL
BEGIN
DECLARE @SQL_ALTER1 NVARCHAR(4000) = '
ALTER TABLE EDAA_STG.FCT_DLY_STR_ITMSKU_SLS_OUT  ADD CONSTRAINT PK_FCT_DLY_STR_ITMSKU_SLS_OUT
PRIMARY KEY NONCLUSTERED
(
		DT_SK ASC,
		GEO_HIST_SK ASC,
...
SELECT	CAST(REPLACE(A.BUS_DT,'-','') AS INT)	AS DT_SK,
		ISNULL(GEO_HIST_SK, -1) 				AS GEO_HIST_SK,
		ISNULL(PROD_HIST_SK,-1)					AS PROD_HIST_SK,
		'X'										AS MPRS_PRCS_ID,
		A.UPC_ID								AS ITM_SKU,
		A.UT_ID									AS STR_ID,
		PKY_ID									AS FNC_LVL3_PKY_ID,
		MPRS_CT_ID								AS FNC_LVL2_MPRS_CTGRY_ID,
		'X'										AS SBT_FLAG,
		SUM(T_SL_AM)							AS SLS_AMT	,
		SUM(T_SL_QT)							AS SLS_QTY	,
		SUM(T_WGT_QT)							AS SLS_WGT	,
		SUM(T_PROMO_SL_AM)						AS PRM_TO_SLS_AMT	,
		SUM(T_PROMO_SL_QT)						AS PRM_TO_SLS_QTY	,
		SUM(T_PROMO_WGT_QT)						AS PRM_TO_SLS_WGT	,
		SUM(T_PROMO_MKDN_AM)					AS PRM_TO_SLS_MKDN_AMT	,
		SUM(T_CLRN_SL_AM)						AS CLRNC_SLS_AMT	,
		SUM(T_CLRN_SL_QT)						AS CLRNC_QTY	,
		SUM(T_CLRN_WGT_QT)						AS CLRNC_WGT	,
		SUM(T_CLRN_MKDN_AM)						AS CLRNC_MKDN_AMT	,
		SUM(T_CGS_P_CST_AM)						AS COGS_AMT	,
		SUM(T_CGS_FNC_CHRG_AM)					AS COGS_FNC_CHRG_AMT	,
		SUM(T_CGS_FNC_CR_AM)					AS COGS_FNC_CR_AMT	,
		SUM(T_CGS_STG_CHRG_AM)					AS COGS_STRGE_CHRG_AMT	,
		SUM(T_DMGN_AM)							AS DRCT_MGN_AMT	,
		SUM(T_PROMO_DMGN_AM)					AS PRM_TO_SLS_DRCT_MGN_AMT	,
		SUM(T_CLRN_DMGN_AM)						AS CLRNC_DRCT_MGN_AMT	,
		Sum(T_CGS_FRT_AM)						AS COGS_FRT_AMT	,
		SUM(T_CGS_DF_HDL_AM)					AS COGS_DIST_CNTR_HNDL_AMT	,
		@NEW_AUD_SKY							AS AUD_INS_SK,
		NULL									AS AUD_UPD_SK

FROM EDAA_STG.UT_UPC_DLY_SL_HST_INITIAL AS A
...
LEFT OUTER JOIN (SELECT PKY_ID,
						P_UPC_ID,
						MPRS_CT_ID,
						P_UPC_DEF_EFF_DT,
						P_UPC_DEF_EXPR_DT
				FROM 	[EDAA_STG].[P_UPC_HST] --VPROD_P_HST.P_UPC_HST
			--WHERE 	IS_CURR<>0

					) AS P1
	ON   P1.P_UPC_ID = A.UPC_ID
	AND  cast(A.BUS_DT as date) BETWEEN P1.P_UPC_DEF_EFF_DT AND ISNULL(P1.P_UPC_DEF_EXPR_DT, CAST('2099-01-01' AS DATE))
...
INSERT INTO EDAA_STG.FCT_DLY_STR_ITMSKU_SLS_IN
select
DT_SK,
GEO_HIST_SK,
PROD_HIST_SK,
...
WHERE SLS_AMT<>0
      ...
      or COGS_FRT_AMT<>0
      or COGS_DIST_CNTR_HNDL_AMT<>0; --where  GEO_HIST_SK<>-1 and PROD_HIST_SK<>-1
...
--SELECT @NBR_OF_RW_ISRT = COUNT(1)  FROM EDAA_STG.FCT_DLY_STR_ITMSKU_SLS_IN WHERE AUD_INS_SK = @NEW_AUD_SKY
--SELECT @NBR_OF_RW_UDT  = COUNT(1)  FROM EDAA_STG.FCT_DLY_STR_ITMSKU_SLS_IN WHERE AUD_UPD_SK = @NEW_AUD_SKY
...
ALTER TABLE EDAA_DW.FCT_DLY_STR_ITMSKU_SLS    SWITCH PARTITION '+CAST(@PNUMBER AS VARCHAR(20))    +' TO EDAA_STG.FCT_DLY_STR_ITMSKU_SLS_OUT PARTITION '    +CAST(@PNUMBER AS VARCHAR(20))+';
ALTER TABLE EDAA_STG.FCT_DLY_STR_ITMSKU_SLS_IN SWITCH PARTITION '+CAST(@PNUMBER AS VARCHAR(20))    +' TO EDAA_DW.FCT_DLY_STR_ITMSKU_SLS PARTITION '        +CAST(@PNUMBER AS VARCHAR(20))+';'
EXEC SP_EXECUTESQL @SQL

--PERFORM THE CLEAN-UP
TRUNCATE TABLE EDAA_STG.FCT_DLY_STR_ITMSKU_SLS_OUT;
TRUNCATE TABLE EDAA_STG.FCT_DLY_STR_ITMSKU_SLS_IN;
...
BEGIN CATCH
DECLARE @ERROR_PROCEDURE_NAME AS VARCHAR(60) = '[EDAA_ETL].[PROC_FCT_DLY_STR_ITMSKU_SLS_INITIAL_TABLELOAD]'
