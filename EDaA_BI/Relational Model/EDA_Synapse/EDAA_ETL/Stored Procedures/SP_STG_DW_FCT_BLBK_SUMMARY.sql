...
CREATE PROC [EDAA_ETL].[SP_STG_DW_FCT_BLBK_SUMMARY] AS

/*
 =============================================
 Author:        Core Build Team
 Create date:   12-july-2022
 Description:   Stored proc load FCT_BLBK_SUMMARY table.
 =============================================
*/
...
EXEC EDAA_CNTL.SP_AUDIT_DATA_LOAD_START
 @EXEC_LYR  = 'EDAA_DW'
,@EXEC_JOB  = 'SP_STG_DW_FCT_BLBK_SUMMARY'
,@SRC_ENTY  = 'EDAA_STG.FCT_BLBK_SUMMARY'
,@TGT_ENTY = 'FCT_BLBK_SUMMARY'
...
INSERT INTO EDAA_DW.FCT_BLBK_SUMMARY
(
		Dt_Sk,
		Geo_Hist_Sk,
		Prod_Hist_Sk,
		CostOfGoodsReductionAmount,
		Bat_Prcs_Tms,
		Bat_Prcs_Id,
		Aud_Ins_Sk,
		Aud_Upd_Sk
...
SELECT
		...
		BLBK.CostOfGoodsReductionAmount,
		BLBK.Bat_Prcs_Tms,
		BLBK.Bat_Prcs_Id,
		@NEW_AUD_SKY AS Aud_Ins_Sk,
		NULL AS Aud_Upd_Sk

FROM  EDAA_STG.FCT_BLBK_SUMMARY BLBK
LEFT JOIN  EDAA_DW.DIM_GEO GEO ON GEO.Str_Id = BLBK.Str_Id AND GEO.IS_Curr_Ind=1
LEFT JOIN  EDAA_DW.DIM_PROD PROD ON PROD.Itm_Sku=BLBK.Itm_Sku AND PROD.IS_Curr_Ind=1
LEFT JOIN  EDAA_DW.Dim_Dly_Cal CAL ON BLBK.Dt_Sk=CAL.Dt_Sk

BEGIN
SELECT @NBR_OF_RW_ISRT = COUNT_BIG(1)  FROM EDAA_DW.FCT_BLBK_SUMMARY WHERE Aud_Ins_Sk = @NEW_AUD_SKY
SELECT @NBR_OF_RW_UDT  = COUNT_BIG(1)  FROM EDAA_DW.FCT_BLBK_SUMMARY WHERE Aud_Upd_Sk = @NEW_AUD_SKY
...
BEGIN CATCH
DECLARE @ERROR_PROCEDURE_NAME AS VARCHAR(60) = 'EDAA_CNTL.SP_STG_DW_FCT_BLBK'
