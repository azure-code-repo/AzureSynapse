...
CREATE PROC [EDAA_ETL].[PROC_FCT_DLY_STR_ITMSKU_INV_TABLELOAD] AS
...
SELECT	A.STR_ID									AS STR_ID,
		...
		CASE WHEN A.INV_CLRNC_IND = 'Y' THEN	0
			 WHEN A.INV_CLRNC_IND = 'N' THEN	1
			 ELSE  NULL		END						AS INV_CLRNC_IND
FROM EDAA_STG.UT_UPC_DLY_INV_HST_INCL AS A
)
update EDAA_DW.FCT_DLY_STR_ITMSKU_INV
set
 INV_ROW_END_DT = src.INV_ROW_END_DT,
 INV_EACH_AMT = src.INV_EACH_AMT,
 INV_FNC_CHRG_AMT = src.INV_FNC_CHRG_AMT,
 INV_FNC_CR_AMT	= src.INV_FNC_CR_AMT,
 INV_STRGE_CHRG_AMT	= src.INV_STRGE_CHRG_AMT,
 INV_FRT_AMT = src.INV_FRT_AMT,
 INV_DIST_CNTR_HNDL_AMT	= src.INV_DIST_CNTR_HNDL_AMT,
 INV_AMT = src.INV_AMT,
 INV_QTY = src.INV_QTY,
 INV_CLRNC_IND = src.INV_CLRNC_IND,
 AUD_UPD_SK = @NEW_AUD_SKY
from EDAA_DW.FCT_DLY_STR_ITMSKU_INV tgt, incl_data src
where
tgt.STR_ID = src.STR_ID and
tgt.ITM_SKU = src.ITM_SKU and
tgt.INV_ROW_START_DT = src.INV_ROW_START_DT
--and cast(tgt.INV_ROW_END_DATE as date)= cast('2099-01-01' as date)
;

print('-- Update records complete for STR_ID, ITM_SKU, INV_ROW_START_DT --')
...
select * from incl_data A
where rtrim(cast(A.STR_ID as char))+'-'+rtrim(cast(A.ITM_SKU as char))+'-'+rtrim(cast(cast(A.INV_ROW_START_DT as date) as varchar)) not in
(select rtrim(cast(B.STR_ID as char))+'-'+rtrim(cast(B.ITM_SKU as char))+'-'+rtrim(cast(cast(B.INV_ROW_START_DT as date) as varchar)) from EDAA_DW.FCT_DLY_STR_ITMSKU_INV B)
;

EXEC EDAA_ETL.PROC_FCT_DLY_STR_ITMSKU_INV_INITIAL_POSTPROCESS;
...
BEGIN CATCH
DECLARE @ERROR_PROCEDURE_NAME AS VARCHAR(60) = '[EDAA_ETL].[PROC_FCT_DLY_STR_ITMSKU_INV_TABLELOAD]'
