CREATE PROC [EDAA_ETL].[PROC_FCT_DLY_STR_ITMSKU_INV_HST_INITIAL_TABLELOAD] AS
...
EXEC EDAA_CNTL.SP_AUDIT_DATA_LOAD_START
 @EXEC_LYR  = 'EDAA_DW'
,@EXEC_JOB  = 'PROC_FCT_DLY_STR_ITMSKU_INV_HST_INITIAL_TABLELOAD'
...
FROM  EDAA_STG.UT_UPC_DLY_INV_HST_INITIAL




/*
IF OBJECT_ID('EDAA_STG.PK_FCT_DLY_STR_ITMSKU_INV_HST_IN') IS NULL
BEGIN
DECLARE @SQL_ALTER NVARCHAR(4000) = '
ALTER TABLE EDAA_STG.FCT_DLY_STR_ITMSKU_INV_HST_IN  ADD CONSTRAINT PK_FCT_DLY_STR_ITMSKU_INV_HST_IN
...
@SCHEMA = 'EDAA_DW',
@TABLENAME='FCT_DLY_STR_ITMSKU_INV_HST',
...
SELECT	CAST(REPLACE(A.BUS_DT,'-','') AS INT)	AS DT_SK,
		...
		A.UPC_ID								AS ITM_SKU,
		A.UT_ID									AS STR_ID,
		sum(UPC_INV_MPC_EA_AM)					AS INV_EACH_AMT,
		sum(T_UPC_INV_FNC_CHRG_AM)				AS INV_FNC_CHRG_AMT,
		sum(T_UPC_INV_FNC_CR_AM)				AS INV_FNC_CR_AMT,
		sum(T_UPC_INV_STG_CHRG_AM)				AS INV_STRGE_CHRG_AMT,
		sum(T_UPC_INV_FRT_AM)					AS INV_FRT_AMT,
		sum(T_UPC_INV_DF_HDL_AM)				AS INV_DIST_CNTR_HNDL_AMT,
		sum(T_UPC_INV_AM)						AS INV_AMT,
		sum(T_UPC_INV_EA_QT)					AS INV_QTY,
		CASE WHEN A.T_UPC_INV_CLRN_FLG='Y' THEN 0 WHEN A.T_UPC_INV_CLRN_FLG='N' THEN 1 ELSE NULL END	AS INV_CLRNC_IND,
		@NEW_AUD_SKY							AS AUD_INS_SK,
		NULL									AS AUD_UPD_SK

FROM EDAA_STG.UT_UPC_DLY_INV_HST_INITIAL AS A
...
where REPLACE(A.BUS_DT,'-','')>= format(dateadd(d,-180,getdate()),'yyyyMMdd')
...
GROUP BY
		...
		PROD_HIST_SK, UPC_ID,
		A.UT_ID,
		(CASE WHEN A.T_UPC_INV_CLRN_FLG='Y' THEN 0 WHEN A.T_UPC_INV_CLRN_FLG='N' THEN 1 ELSE NULL END)
		)
INSERT INTO EDAA_STG.FCT_DLY_STR_ITMSKU_INV_HST_IN
select
DT_SK,
PROD_HIST_SK,
GEO_HIST_SK,
...
--SELECT @NBR_OF_RW_ISRT = COUNT(1)  FROM EDAA_STG.FCT_DLY_STR_ITMSKU_INV_HST_IN WHERE AUD_INS_SK = @NEW_AUD_SKY
--SELECT @NBR_OF_RW_UDT  = COUNT(1)  FROM EDAA_STG.FCT_DLY_STR_ITMSKU_INV_HST_IN WHERE AUD_UPD_SK = @NEW_AUD_SKY
...
ALTER TABLE EDAA_DW.FCT_DLY_STR_ITMSKU_INV_HST    SWITCH PARTITION '+CAST(@PNUMBER AS VARCHAR(20))    +' TO EDAA_STG.FCT_DLY_STR_ITMSKU_INV_HST_OUT PARTITION '    +CAST(@PNUMBER AS VARCHAR(20))+';
ALTER TABLE EDAA_STG.FCT_DLY_STR_ITMSKU_INV_HST_IN SWITCH PARTITION '+CAST(@PNUMBER AS VARCHAR(20))    +' TO EDAA_DW.FCT_DLY_STR_ITMSKU_INV_HST PARTITION '        +CAST(@PNUMBER AS VARCHAR(20))+';'
EXEC SP_EXECUTESQL @SQL

--PERFORM THE CLEAN-UP
TRUNCATE TABLE EDAA_STG.FCT_DLY_STR_ITMSKU_INV_HST_OUT;
TRUNCATE TABLE EDAA_STG.FCT_DLY_STR_ITMSKU_INV_HST_IN;
...
BEGIN CATCH
DECLARE @ERROR_PROCEDURE_NAME AS VARCHAR(60) = '[EDAA_ETL].[PROC_FCT_DLY_STR_ITMSKU_INV_HST_INITIAL_TABLELOAD]'
