/****** Object:  StoredProcedure [EDAA_ETL].[PROC_FCT_DLY_COST_MPRS_SLS_DLY_LOAD_UAT]    Script Date: 3/1/2023 5:04:56 AM ******/

CREATE PROC [EDAA_ETL].[PROC_FCT_DLY_COST_MPRS_SLS_DLY_LOAD] AS
BEGIN TRY
...
DECLARE @ERROR_PROCEDURE_NAME AS VARCHAR(60) = 'EDAA_ETL.PROC_FCT_DLY_COST_MPRS_SLS_DLY_LOAD'
...
EXEC EDAA_CNTL.SP_AUDIT_DATA_LOAD_START
 @EXEC_LYR  = 'EDAA_PRES'
,@EXEC_JOB  = 'PROC_FCT_DLY_COST_MPRS_SLS_DLY_LOAD'
,@SRC_ENTY  = 'FCT_PRODUCT_MPRS_COSTRCV_TY_LY_FSC_42DAYS'
,@TGT_ENTY = 'FCT_DLY_STR_ITMSKU_MPRS_COSTRCV_SLS'
,@NEW_AUD_SKY = @NEW_AUD_SKY OUTPUT;


TRUNCATE TABLE [EDAA_PRES].[FCT_DLY_STR_ITMSKU_MPRS_COSTRCV_SLS]
...
;WITH CTE AS
(
SELECT DISTINCT PRE_DEF_ID FROM EDAA_PRES.V_DIM_PRE_DEF_CAL_TYP a WHERE a.AGG_TYP = 'DLY'
)
INSERT INTO [EDAA_PRES].[FCT_DLY_STR_ITMSKU_MPRS_COSTRCV_SLS]
...
Getdate()
FROM
[EDAA_PRES].[FCT_DLY_STR_ITMSKU_SLS_DLY] b
INNER JOIN CTE ON b.PRE_DEF_ID=CTE.PRE_DEF_ID
INNER JOIN EDAA_DW.DIM_GEO c
       ON b.STR_ID = c.STR_ID and c.Is_Curr_ind = 1
INNER JOIN [EDAA_PRES].[FCT_PRODUCT_MPRS_COSTRCV_TY_LY_FSC_42DAYS] a
...
from [EDAA_PRES].[FCT_PRODUCT_MPRS_COSTRCV_TY_LY_FSC_42DAYS] as t
INNER JOIN [EDAA_PRES].[FCT_DLY_STR_ITMSKU_SLS_DLY] AS b
...
FROM
[EDAA_PRES].[FCT_DLY_STR_ITMSKU_SLS_DLY] b
INNER JOIN CTE1 ON b.PRE_DEF_ID=CTE1.PRE_DEF_ID
INNER JOIN EDAA_DW.DIM_GEO c
       ON b.STR_ID = c.STR_ID and c.Is_Curr_ind = 1
LEFT JOIN [EDAA_PRES].[FCT_PRODUCT_MPRS_COSTRCV_TY_LY_FSC_42DAYS] a
                      ON a.PRE_DEF_ID = b.PRE_DEF_ID
					  AND a.GEO_HIST_SK = c.GEO_HIST_SK
                      AND a.ITM_SKU = b.ITM_SKU
LEFT JOIN CTE_AVG AS G ON B.ITM_SKU=G.ITM_SKU AND B.PRE_DEF_ID=G.PRE_DEF_ID
WHERE A.GEO_HIST_SK IS NULL AND a.ITM_SKU	IS NULL AND a.PRE_DEF_ID IS NULL
END
 --UPDATE STATISTICS
 UPDATE STATISTICS [EDAA_PRES].[FCT_DLY_STR_ITMSKU_MPRS_COSTRCV_SLS];
...
SET @ERROR_PROCEDURE_NAME = 'EDAA_ETL.PROC_FCT_DLY_COST_MPRS_SLS_DLY_LOAD'
--SET @ERROR_LINE
--SET @ERROR_MSG
