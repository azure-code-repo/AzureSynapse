/****** Object:  StoredProcedure [EDAA_ETL].[PROC_FCT_DLY_COST_RCV_TY_LY_LOAD_IPOP]    Script Date: 3/15/2023 2:39:09 AM ******/

CREATE PROC [EDAA_ETL].[PROC_FCT_DLY_COST_RCV_TY_LY_LOAD_IPOP]  @From_DT_SK [INT],@TO_DT_SK [INT] AS
BEGIN TRY
...
DECLARE @ERROR_PROCEDURE_NAME AS VARCHAR(60) = 'EDAA_ETL.PROC_FCT_DLY_COST_RCV_TY_LY_LOAD_IPOP'
...
WHERE T.DT_SK>=@From_DT_SK AND T.DT_SK<=@TO_DT_SK
),
Ipop_load
AS (
SELECT
...
-- FROM EDAA_PRES.FCT_PRODUCT_COST_RCV_TY_LY_FSC WHERE DT_SK >= @From_DT_SK AND DT_SK <=@TO_DT_SK
-- )
...
SET @ERROR_PROCEDURE_NAME = 'EDAA_ETL.PROC_FCT_DLY_COST_RCV_TY_LY_LOAD_IPOP'
--SET @ERROR_LINE
--SET @ERROR_MSG
