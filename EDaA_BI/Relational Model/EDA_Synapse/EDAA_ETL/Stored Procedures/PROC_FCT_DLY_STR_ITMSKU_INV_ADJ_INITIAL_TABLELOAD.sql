/****** Object:  StoredProcedure [EDAA_ETL].[PROC_FCT_DLY_STR_ITMSKU_INV_ADJ_INITIAL_TABLELOAD]    Script Date: 10/25/2021 9:24:22 AM ******/
CREATE PROC [EDAA_ETL].[PROC_FCT_DLY_STR_ITMSKU_INV_ADJ_INITIAL_TABLELOAD] AS
...
EXEC EDAA_CNTL.SP_AUDIT_DATA_LOAD_START
 @EXEC_LYR  = 'EDAA_DW'
,@EXEC_JOB  = 'PROC_FCT_DLY_STR_ITMSKU_INV_ADJ_INITIAL_TABLELOAD'
,@SRC_ENTY  = 'UT_UPC_DLY_INV_TN_DTL_HST_INITIAL'
,@TGT_ENTY = 'FCT_DLY_STR_ITMSKU_INV_ADJ'
...
FROM  EDAA_STG.UT_UPC_DLY_INV_TN_DTL_HST_INITIAL
...
@SCHEMA = 'EDAA_DW',
@TABLENAME='FCT_DLY_STR_ITMSKU_INV_ADJ',
@PARTITIONVALUE = @PARTITIONDATESTARTKEY,
@PARTITIONNUMBER = @PNUMBER  OUTPUT ;
SELECT @PNUMBER

IF OBJECT_ID('EDAA_DW.FCT_DLY_STR_ITMSKU_INV_ADJ_OUT') IS NOT NULL
BEGIN
    DROP TABLE EDAA_DW.FCT_DLY_STR_ITMSKU_INV_ADJ_OUT
END


CREATE TABLE EDAA_DW.FCT_DLY_STR_ITMSKU_INV_ADJ_OUT
WITH
(
...
ALTER TABLE EDAA_DW.FCT_DLY_STR_ITMSKU_INV_ADJ_OUT ADD CONSTRAINT [PK_FCT_DLY_STR_ITMSKU_INV_ADJ_OUT] PRIMARY KEY NONCLUSTERED
	(
		[PROD_HIST_SK] ASC,
		[GEO_HIST_SK] ASC,
		[DT_SK] ASC,
...
IF OBJECT_ID('EDAA_DW.FCT_DLY_STR_ITMSKU_INV_ADJ_IN') IS NOT NULL
BEGIN
    DROP TABLE EDAA_DW.FCT_DLY_STR_ITMSKU_INV_ADJ_IN
END

CREATE TABLE EDAA_DW.FCT_DLY_STR_ITMSKU_INV_ADJ_IN
WITH
(
	DISTRIBUTION = HASH ( DT_SK ),
	CLUSTERED COLUMNSTORE INDEX,
...
ALTER TABLE EDAA_DW.FCT_DLY_STR_ITMSKU_INV_ADJ_IN ADD CONSTRAINT [PK_FCT_DLY_STR_ITMSKU_INV_ADJ_IN] PRIMARY KEY NONCLUSTERED
	(
		[PROD_HIST_SK] ASC,
		[GEO_HIST_SK] ASC,
		[DT_SK] ASC,
...
IF OBJECT_ID('EDAA_DW.PK_FCT_DLY_STR_ITMSKU_INV_ADJ_IN') IS NULL
BEGIN
DECLARE @SQL_ALTER NVARCHAR(4000) = '
ALTER TABLE EDAA_DW.FCT_DLY_STR_ITMSKU_INV_ADJ_IN  ADD CONSTRAINT PK_FCT_DLY_STR_ITMSKU_INV_ADJ_IN
...
SELECT	ISNULL(PROD_HIST_SK,-1)					AS PROD_HIST_SK,
		...
		UPC_ID									AS ITM_SKU,
		CAST(SUBSTRING(REPLACE(A.BUS_DT ,'-',''),1,8) AS INT)	AS DT_SK,
		ISNULL(INV_ADJ_SK, -1)					AS INV_ADJ_SK,
		A.UT_ID									AS STR_ID,
		T_INV_TN_QT								AS INV_ADJSTD_QTY,
		T_INV_TN_AM								AS INV_ADJSTD_AMT,
		@NEW_AUD_SKY							AS AUD_INS_SK,
		NULL									AS AUD_UPD_SK
FROM EDAA_STG.UT_UPC_DLY_INV_TN_DTL_HST_INITIAL AS A
...
LEFT OUTER JOIN (SELECT INV_ADJ_SK, TXN_TYP, ADJ_TYP_ID FROM EDAA_DW.DIM_INV_ADJ) AS INV_ADJ
				 ON INV_ADJ.TXN_TYP= A.INV_TN_TYP_ID AND INV_ADJ.ADJ_TYP_ID =A.INV_TN_TYP_CT_ID
)



INSERT INTO EDAA_DW.FCT_DLY_STR_ITMSKU_INV_ADJ_IN
SELECT	PROD_HIST_SK,
		GEO_HIST_SK,
		ITM_SKU,
		DT_SK,
		...
		AUD_INS_SK,
		AUD_UPD_SK
from initial_data ; --where  GEO_HIST_SK<>-1 and PROD_HIST_SK<>-1
...
INSERT INTO  EDAA_STG.FCT_DLY_STR_ITMSKU_INV_ADJ_ERRORREC
select * from initial_data where  GEO_HIST_SK=-1 or PROD_HIST_SK=-1
;
...
--SELECT @NBR_OF_RW_ISRT = COUNT(1)  FROM EDAA_DW.FCT_DLY_STR_ITMSKU_INV_ADJ_IN WHERE AUD_INS_SK = @NEW_AUD_SKY
--SELECT @NBR_OF_RW_UDT  = COUNT(1)  FROM EDAA_DW.FCT_DLY_STR_ITMSKU_INV_ADJ_IN WHERE AUD_UPD_SK = @NEW_AUD_SKY
...
ALTER TABLE EDAA_DW.FCT_DLY_STR_ITMSKU_INV_ADJ    SWITCH PARTITION '+CAST(@PNUMBER AS VARCHAR(20))    +' TO EDAA_DW.FCT_DLY_STR_ITMSKU_INV_ADJ_OUT PARTITION '    +CAST(@PNUMBER AS VARCHAR(20))+';
ALTER TABLE EDAA_DW.FCT_DLY_STR_ITMSKU_INV_ADJ_IN  SWITCH PARTITION '+CAST(@PNUMBER AS VARCHAR(20))    +' TO EDAA_DW.FCT_DLY_STR_ITMSKU_INV_ADJ PARTITION '        +CAST(@PNUMBER AS VARCHAR(20))+';'
EXEC SP_EXECUTESQL @SQL

--PERFORM THE CLEAN-UP
TRUNCATE TABLE EDAA_DW.FCT_DLY_STR_ITMSKU_INV_ADJ_OUT;
TRUNCATE TABLE EDAA_DW.FCT_DLY_STR_ITMSKU_INV_ADJ_IN;
...
BEGIN CATCH
DECLARE @ERROR_PROCEDURE_NAME AS VARCHAR(60) = '[EDAA_ETL].[PROC_FCT_DLY_STR_ITMSKU_INV_ADJ_INITIAL_TABLELOAD]'
