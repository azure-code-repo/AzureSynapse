CREATE PROC [ETL].[PROC_FCT_DLY_SL_MB_TXN_P_SUB_CT_INCR_TABLELOAD] AS
...
DECLARE @ERROR_PROCEDURE_NAME AS VARCHAR(60) = 'ETL.PROC_FCT_DLY_SL_MB_TXN_P_SUB_CT_INCR_TABLELOAD' ;
...
EXEC ETL.AUDIT_DATA_LOAD_START
 @EXEC_LYR  = 'DM'
,@EXEC_JOB  = 'PROC_FCT_DLY_SL_MB_TXN_P_SUB_CT_INCR_TABLELOAD'
,@SRC_ENTY  = 'DLY_P_MDS_HCY_LVL_TN INCL'
...
FROM  STG.DLY_P_MDS_HCY_LVL_TN_HST
...
DECLARE
		@PARTITIONDATE 			INT		,
		@PROCESSINGDAY 			DATE	,
		@PARTITIONDATESTART 	DATE	,
		@PARTITIONDATEEND 	 	DATE	,
		@PARTITIONDATESTARTKEY 	INT		,
		@PARTITIONDATEENDKEY  	INT
...
PRINT('LATEARRIVINGDIMENSION: DIM_CHNL')
...
SELECT
DISTINCT
FACT_DATA_FEED.CHNL_BUS_KEY,
CASE WHEN CHNL.CHNL_BUS_KEY IS NULL THEN 1 ELSE 0 END AS IS_CHNL_MISSING ,
...
WHERE
(CHNL.CHNL_BUS_KEY IS NULL OR GEO.UT_ID IS NULL)
...
SELECT
		  ...
		  ,CONVERT(INTEGER,-1)    			SHOP_CHNL_CT
		  ,CONVERT(INTEGER,0) 	  			SHOP_CHNL_GRP_CT
...
SELECT
		   (ROW_NUMBER() OVER (ORDER BY UT_ID)) + @NEXT_GEO_HST_SKY 		AS CHNL_HST_SKY
...
FROM        STG.DLY_P_MDS_HCY_LVL_TN_HST
WHERE CAST(DAY_DT AS DATE)   BETWEEN @PROCESSINGDAY AND @PROCESSINGENDDATE
