CREATE PROC [ETL].[PROC_FCT_DLY_SL_P_SUB_CT_SALESLOAD] AS
BEGIN TRY
...
DECLARE @ERROR_PROCEDURE_NAME AS VARCHAR(60) = 'ETL.PROC_FCT_DLY_SL_P_SUB_CT_SALESLOAD' ;
...
EXEC ETL.AUDIT_DATA_LOAD_START
 @EXEC_LYR  = 'DM'
,@EXEC_JOB  = 'PROC_FCT_DLY_SL_P_SUB_CT_SALESLOAD'
,@SRC_ENTY  = 'DLY_UT_MPRS_CT_HST_SALES'
...
FROM       STG.DLY_UT_MPRS_CT_HST_SALES AA
INNER  JOIN (
			SELECT 	PKY_ID,
					MPRS_CT_ID,
					MJR_P_SUB_CT_ID
...
FROM
	...
	WHERE
	MISSING_DIM_P_L3.P_SUB_CT_ID= DIM.P_SUB_CT_ID
	)
...
(SELECT
		...
		FROM       STG.DLY_UT_MPRS_CT_HST_SALES         AS AA
            INNER JOIN (
...
INSERT INTO DM.FCT_DLY_SL_P_SUB_CT  (
	...
	SLS_FNC_SCN_QTY,
	SLS_FNC_COMP,
	SLS_FNC_NCOMP , @NEW_AUD_SKY     AS AUD_INS_SKY
	FROM NEWDATA SOURCE
