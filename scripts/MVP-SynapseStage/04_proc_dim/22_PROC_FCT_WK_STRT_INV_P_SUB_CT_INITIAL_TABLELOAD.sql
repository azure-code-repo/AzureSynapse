...
CREATE PROC [ETL].[PROC_FCT_WK_STRT_INV_P_SUB_CT_INITIAL_TABLELOAD] AS
...
EXEC ETL.AUDIT_DATA_LOAD_START
 @EXEC_LYR  = 'DM'
,@EXEC_JOB  = 'PROC_FCT_WK_STRT_INV_P_SUB_CT_INITIAL_TABLELOAD'
,@SRC_ENTY  = 'WK_UT_MPRS_CT_HST AND WK_DC_MPRS_CT_HST'
...
SELECT
@NUMBEROFPARTITIONS = DATEDIFF(M, MIN(WK_END_DT),MAX(WK_END_DT))+1,
@NEXTPROCESSINGSTARTDATE = MIN(WK_END_DT),
@MAXPROCESSINGDATE = MAX(WK_END_DT)
FROM  STG_EDW.WK_UT_MPRS_CT_HST



IF OBJECT_ID('DM_EDW.FCT_WK_STRT_INV_P_SUB_CT_OUT') IS NOT NULL
BEGIN
    DROP TABLE DM_EDW.FCT_WK_STRT_INV_P_SUB_CT_OUT
END



CREATE TABLE DM_EDW.FCT_WK_STRT_INV_P_SUB_CT_OUT
WITH
(
...
AS
SELECT *
FROM    DM.FCT_WK_STRT_INV_P_SUB_CT
WHERE 1=2
OPTION (LABEL = 'CTAS : PARTITION SWITCH IN : UPDATE')
;

IF OBJECT_ID('DM_EDW.FCT_WK_STRT_INV_P_SUB_CT_IN') IS NOT NULL
BEGIN
    DROP TABLE DM_EDW.FCT_WK_STRT_INV_P_SUB_CT_IN
END

CREATE TABLE DM_EDW.FCT_WK_STRT_INV_P_SUB_CT_IN
WITH
(
	DISTRIBUTION = HASH ( UT_ID ),
	CLUSTERED COLUMNSTORE INDEX,
...
FROM
	(
		SELECT DISTINCT
		REPLACE(CAST(WK_END_DT  AS DATE),'-','') AS WK_END_DT
		FROM  STG_EDW.WK_UT_MPRS_CT_HST
		WHERE
		CAST(WK_END_DT AS DATE)   BETWEEN @PROCESSINGDAY AND @PROCESSINGENDDATE
	) AS INCL
WHERE
TM_SKY = WK_END_DT
)
...
IF OBJECT_ID('DM_EDW.PK_FCT_WK_STRT_INV_P_SUB_CT_IN') IS NULL
BEGIN
DECLARE @SQL_ALTER NVARCHAR(4000) = '
ALTER TABLE DM_EDW.FCT_WK_STRT_INV_P_SUB_CT_IN  ADD CONSTRAINT PK_FCT_WK_STRT_INV_P_SUB_CT_IN
...
@SCHEMA = 'DM',
@TABLENAME='FCT_WK_STRT_INV_P_SUB_CT',
...
INSERT INTO DM_EDW.FCT_WK_STRT_INV_P_SUB_CT_IN


SELECT
		ISNULL(GEO_HST_SKY, -1) 			AS GEO_HST_SKY,
		REPLACE(WK_END_DT,'-','') 		AS TM_SKY,
		ISNULL(BYR_HST_SKY, -1) 			AS BYR_HST_SKY,
		MJR_P_SUB_CT_ID 					AS P_SUB_CT_ID,
		MPRS_CT_ID 							AS FNC_MPRS_CT_ID,
		PKY_ID 								AS FNC_PKY_ID,
		A.UT_ID								AS UT_ID,
		TG_INV_CST_AM						AS TG_INV_CST_AM,
		TG_CLRN_INV_CST_AM					AS TG_CLRN_INV_CST_AM,
		TG_CLRN_INV_QT 						AS INV_CLRN_QTY,
		TG_INV_QT							AS TG_INV_QT,
...
SELECT
			MJR_P_SUB_CT_ID,
			AA.WK_END_DT  				AS WK_END_DT,
			UT_ID,
			AA.PKY_ID					AS PKY_ID,
			AA.MPRS_CT_ID  				AS MPRS_CT_ID,
			SUM(TG_INV_CST_AM )      	AS TG_INV_CST_AM,
			SUM(TG_INV_QT )          	AS TG_INV_QT,
			SUM(TG_CLRN_INV_CST_AM ) 	AS TG_CLRN_INV_CST_AM,
			SUM(TG_CLRN_INV_QT )     	AS TG_CLRN_INV_QT
FROM   STG_EDW.WK_UT_MPRS_CT_HST AS AA
       INNER JOIN (
...
AND 	CAST(WK_END_DT AS DATE)   BETWEEN @PROCESSINGDAY AND @PROCESSINGENDDATE
GROUP  BY MJR_P_SUB_CT_ID,         WK_END_DT,
       UT_ID,AA.PKY_ID	 ,	         AA.MPRS_CT_ID
...
SELECT
			...
			SUM(TG_CLRN_INV_CST_AM ) 	AS TG_CLRN_INV_CST_AM,
			SUM(TG_CLRN_INV_QT )     	AS TG_CLRN_INV_QT
FROM   STG_EDW.WK_DC_MPRS_CT_HST AS AA
       INNER JOIN (
...
SELECT @NBR_OF_RW_ISRT = COUNT(1)  FROM DM_EDW.FCT_WK_STRT_INV_P_SUB_CT_IN
WHERE AUD_INS_SKY = @NEW_AUD_SKY
SELECT @NBR_OF_RW_UDT = COUNT(1)  FROM DM_EDW.FCT_WK_STRT_INV_P_SUB_CT_IN
...
ALTER TABLE DM.FCT_WK_STRT_INV_P_SUB_CT    SWITCH PARTITION '+CAST(@PNUMBER AS VARCHAR(20))    +' TO DM_EDW.FCT_WK_STRT_INV_P_SUB_CT_OUT PARTITION '    +CAST(@PNUMBER AS VARCHAR(20))+';
ALTER TABLE DM_EDW.FCT_WK_STRT_INV_P_SUB_CT_IN SWITCH PARTITION '+CAST(@PNUMBER AS VARCHAR(20))    +' TO DM.FCT_WK_STRT_INV_P_SUB_CT PARTITION '        +CAST(@PNUMBER AS VARCHAR(20))+';'
EXEC SP_EXECUTESQL @SQL

--PERFORM THE CLEAN-UP
TRUNCATE TABLE DM_EDW.FCT_WK_STRT_INV_P_SUB_CT_OUT;
TRUNCATE TABLE DM_EDW.FCT_WK_STRT_INV_P_SUB_CT_IN;
...
BEGIN CATCH
DECLARE @ERROR_PROCEDURE_NAME AS VARCHAR(60) = '[ETL].[PROC_FCT_WK_STRT_INV_P_SUB_CT_INITIAL_TABLELOAD]'
