...
CREATE PROC [ETL].[PROC_FCT_DLY_SL_PLN_P_SUB_CT_INITIAL_TABLELOAD] AS
...
EXEC ETL.AUDIT_DATA_LOAD_START
 @EXEC_LYR  = 'DM'
,@EXEC_JOB  = 'PROC_FCT_DLY_SL_PLN_P_SUB_CT_INITIAL_TABLELOAD'
,@SRC_ENTY  = 'FNC_SLSPN_MPRS_UT_DAY_INF'
,@TGT_ENTY = 'FCT_DLY_SL_PLN_P_SUB_CT'
...
FROM  STG_EDW.FNC_SLSPN_MPRS_UT_DAY_INF



IF OBJECT_ID('DM_EDW.FCT_DLY_SL_PLN_P_SUB_CT_OUT') IS NOT NULL
BEGIN
    DROP TABLE DM_EDW.FCT_DLY_SL_PLN_P_SUB_CT_OUT
END


CREATE TABLE DM_EDW.FCT_DLY_SL_PLN_P_SUB_CT_OUT
WITH
(
...
AS
SELECT *
FROM    DM.FCT_DLY_SL_PLN_P_SUB_CT
WHERE 1=2
OPTION (LABEL = 'CTAS : PARTITION SWITCH IN : UPDATE')
;

IF OBJECT_ID('DM_EDW.FCT_DLY_SL_PLN_P_SUB_CT_IN') IS NOT NULL
BEGIN
    DROP TABLE DM_EDW.FCT_DLY_SL_PLN_P_SUB_CT_IN
END

CREATE TABLE DM_EDW.FCT_DLY_SL_PLN_P_SUB_CT_IN
WITH
(
	DISTRIBUTION = HASH ( UT_ID ),
	CLUSTERED COLUMNSTORE INDEX,
...
IF OBJECT_ID('DM_EDW.PK_FCT_DLY_SL_PLN_P_SUB_CT_IN') IS NULL
BEGIN
DECLARE @SQL_ALTER NVARCHAR(4000) = '
ALTER TABLE DM_EDW.FCT_DLY_SL_PLN_P_SUB_CT_IN  ADD CONSTRAINT PK_FCT_DLY_SL_PLN_P_SUB_CT_IN
...
@SCHEMA = 'DM',
@TABLENAME='FCT_DLY_SL_PLN_P_SUB_CT',
...
SELECT
				...
				PKY_ID,
				MPRS_CT_ID,
				SUM(TG_SLSPN_AM)                      	AS SLS_FNC_AN_PLN_AM,
                SUM(TG_DMGN_PLN_AM)                   	AS DM_FNC_AN_PLN_AM ,
                SUM(TG_STL_PLN_AM+TG_TWAW_PLN_AM)  		AS INV_SHRK_AN_PLN
FROM		STG_EDW.FNC_SLSPN_MPRS_UT_DAY_INF
...
AND 		DAY_DT    BETWEEN @PROCESSINGDAY AND @PROCESSINGENDDATE
GROUP BY 		UT_ID,				DAY_DT,
				PKY_ID,				MPRS_CT_ID)

INSERT INTO DM_EDW.FCT_DLY_SL_PLN_P_SUB_CT_IN


SELECT
			ISNULL(GEO_HST_SKY, -1) 							AS GEO_HST_SKY		,
			REPLACE(CAST(FNC_SLSPN.DAY_DT  AS DATE),'-','')		AS TM_SKY			,
			ISNULL(BYR_HST_SKY, -1) 							AS BYR_HST_SKY		,
            BYR_EMP.MJR_P_SUB_CT_ID 							AS P_SUB_CT_ID		,
			BYR_EMP.MPRS_CT_ID 									AS FNC_MPRS_CT_ID	,
			BYR_EMP.PKY_ID 										AS FNC_PKY_ID		,
            FNC_SLSPN.UT_ID 		,
			SLS_FNC_AN_PLN_AM		,
			DM_FNC_AN_PLN_AM 		,
			INV_SHRK_AN_PLN			,
...
SELECT
 ...
 WHERE 		INS_DT = (SELECT MAX(INS_DT) FROM DM_EDW.BYR_EMP_PKY_MPRS_CURRPREV_INF)
 AND		MJR_P_SUB_CT_ID IS NOT NULL
)BYR_EMP
	ON 		FNC_SLSPN.PKY_ID=BYR_EMP.PKY_ID
    AND     FNC_SLSPN.MPRS_CT_ID=BYR_EMP.MPRS_CT_ID
...
LEFT JOIN (SELECT	 	BYR_HST_SKY,
	...
	ON BYR.P_SUB_CT_ID = BYR_EMP.MJR_P_SUB_CT_ID


BEGIN
SELECT @NBR_OF_RW_ISRT = COUNT(1)  FROM DM_EDW.FCT_DLY_SL_PLN_P_SUB_CT_IN	 WHERE [AUD_INS_SKY] = @NEW_AUD_SKY
SELECT @NBR_OF_RW_UDT  = COUNT(1)  FROM DM_EDW.FCT_DLY_SL_PLN_P_SUB_CT_IN	 WHERE [AUD_UPD_SKY] = @NEW_AUD_SKY
...
ALTER TABLE DM.FCT_DLY_SL_PLN_P_SUB_CT    SWITCH PARTITION '+CAST(@PNUMBER AS VARCHAR(20))    +' TO DM_EDW.FCT_DLY_SL_PLN_P_SUB_CT_OUT PARTITION '    +CAST(@PNUMBER AS VARCHAR(20))+';
ALTER TABLE DM_EDW.FCT_DLY_SL_PLN_P_SUB_CT_IN SWITCH PARTITION '+CAST(@PNUMBER AS VARCHAR(20))    +' TO DM.FCT_DLY_SL_PLN_P_SUB_CT PARTITION '        +CAST(@PNUMBER AS VARCHAR(20))+';'
EXEC SP_EXECUTESQL @SQL

--PERFORM THE CLEAN-UP
TRUNCATE TABLE DM_EDW.FCT_DLY_SL_PLN_P_SUB_CT_OUT;
TRUNCATE TABLE DM_EDW.FCT_DLY_SL_PLN_P_SUB_CT_IN;
...
END TRY
BEGIN CATCH
DECLARE @ERROR_PROCEDURE_NAME AS VARCHAR(60) = '[ETL].[PROC_FCT_DLY_SL_PLN_P_SUB_CT_INITIAL_TABLELOAD]'
