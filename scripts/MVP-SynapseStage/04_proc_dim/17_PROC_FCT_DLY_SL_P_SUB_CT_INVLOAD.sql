CREATE PROC [ETL].[PROC_FCT_DLY_SL_P_SUB_CT_INVLOAD] AS
...
DECLARE @ERROR_PROCEDURE_NAME AS VARCHAR(60) = 'ETL.PROC_FCT_DLY_SL_P_SUB_CT_INVLOAD' ;
...
EXEC ETL.AUDIT_DATA_LOAD_START
 @EXEC_LYR  = 'DM'
,@EXEC_JOB  = 'PROC_FCT_DLY_SL_P_SUB_CT_INVLOAD'
,@SRC_ENTY  = 'DLY_UT_P_UPC_HST_INV INCR'
...
FROM       STG.DLY_UT_P_UPC_HST_INV AA
INNER  JOIN (
			SELECT 	PKY_ID,
					MPRS_CT_ID,
					MJR_P_SUB_CT_ID
...
(
	SELECT B.MJR_P_SUB_CT_ID AS P_SUB_CT_ID,
          ...
          SUM(A.TG_INV_QT) AS TG_CLRN_INV_QT

          FROM STG.DLY_UT_P_UPC_HST_INV      AS A
		  LEFT JOIN 		  DM.DIM_P PROD
											ON A.P_UPC_ID = PROD.UPC_ID
											AND  A.DAY_DT BETWEEN ISNULL(PROD.VLD_FROM, CAST('2000-01-01' AS DATE)) AND ISNULL(PROD.VLD_TO, CAST('2099-01-01' AS DATE))


		  LEFT JOIN  ( SELECT PKY_ID, MPRS_CT_ID, MJR_P_SUB_CT_ID
					               FROM  DM_EDW.BYR_EMP_PKY_MPRS_CURRPREV_INF
								   WHERE CAST(INS_DT AS DATE) = (SELECT CAST(MAX(INS_DT) AS DATE) AS INS_DT FROM DM_EDW.BYR_EMP_PKY_MPRS_CURRPREV_INF)) AS B

					ON         (PROD.FNC_PKY_ID=B.PKY_ID    AND        PROD.FNC_MPRS_CT_ID=B.MPRS_CT_ID )

			LEFT JOIN
			(	SELECT GEO_HST_SKY, UT_ID, ST_CLS, VLD_FROM, VLD_TO FROM DM.DIM_GEO 	) AS DIMGEO ON (CAST(DIMGEO.UT_ID AS VARCHAR(5)) = CAST(A.UT_ID AS VARCHAR(5)) )

			LEFT JOIN
			DM.DIM_TM AS TM ON TM.DAY_DT = A.DAY_DT

			LEFT JOIN
			(   SELECT BYR_HST_SKY, P_SUB_CT_ID, VLD_FROM, VLD_TO   FROM DM.DIM_BYR_P_SUB_CT ) AS BYR ON (B.MJR_P_SUB_CT_ID = BYR.P_SUB_CT_ID  )

		WHERE A.DAY_DT BETWEEN ISNULL(DIMGEO.VLD_FROM, CAST('2000-01-01' AS DATE)) AND ISNULL(DIMGEO.VLD_TO, CAST('2099-01-01' AS DATE))
		AND  A.DAY_DT BETWEEN ISNULL(BYR.VLD_FROM, CAST('2000-01-01' AS DATE)) AND ISNULL(BYR.VLD_TO, CAST('2099-01-01' AS DATE))
		...
		HAVING  SUM(A.TG_INV_QT) <> 0
)

UPDATE DM.FCT_DLY_SL_P_SUB_CT  SET
	INV_CLRN_QTY = SOURCE.TG_CLRN_INV_QT,
